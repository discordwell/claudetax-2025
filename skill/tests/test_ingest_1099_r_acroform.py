...
# ---------------------------------------------------------------------------
# Real IRS f1099r.pdf template tests (wave 6)
# ---------------------------------------------------------------------------


_REAL_1099_R_PDF: Path = (
...
@pytest.fixture
def real_1099_r(tmp_path: Path) -> Path:
    import shutil

    dst = tmp_path / "1099-r.pdf"
    shutil.copy(_REAL_1099_R_PDF, dst)
    return dst


class TestReal1099RAcroForm:
    def test_real_pdf_exists(self) -> None:
        assert _REAL_1099_R_PDF.exists()

    def test_real_pdf_is_acroform(self, real_1099_r: Path) -> None:
        assert INGESTER.can_handle(real_1099_r) is True

    def test_real_pdf_ingest_succeeds(self, real_1099_r: Path) -> None:
        result = INGESTER.ingest(real_1099_r)
        assert result.success, result.error
        assert result.partial.document_kind == DocumentKind.FORM_1099_R

    def test_field_map_has_real_widget_names(self) -> None:
        reader = pypdf.PdfReader(str(_REAL_1099_R_PDF))
        actual = set(reader.get_fields().keys())
        real_keys = [
            k for k in FORM_1099_R_FIELD_MAP if k.startswith("topmostSubform")
        ]
        assert real_keys, "FORM_1099_R_FIELD_MAP missing real IRS widget keys"
