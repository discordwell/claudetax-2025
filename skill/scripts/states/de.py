...
class DelawarePlugin:
    ...
    def apportion_income(
        ...
        return IncomeApportionment(
            state_source_wages=sourced_or_prorated_wages(return_, "DE", wages, days_in_state)
            if residency != ResidencyStatus.RESIDENT
            else cents(wages),
            ...
            state_source_self_employment=sourced_or_prorated_schedule_c(return_, "DE", se_net, days_in_state)
            if residency != ResidencyStatus.RESIDENT
            else cents(se_net),
