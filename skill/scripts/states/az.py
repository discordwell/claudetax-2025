...
# ---------------------------------------------------------------------------
# Layer 1: AZ Form 140 field dataclass
# ---------------------------------------------------------------------------


@dataclass(frozen=True)
class AZ140Fields:
    """Frozen snapshot of AZ Form 140 line values, ready for rendering.

    Field names correspond to state_specific keys produced by
    ArizonaPlugin.compute().
    """
...
def compute_az140_fields(state_return: StateReturn) -> AZ140Fields:
    """Map StateReturn.state_specific to AZ140Fields."""
...
class ArizonaPlugin:
    ...
    def render_pdfs(
        ...
        _WIDGET_MAP = _REF / "az-140-acroform-map.json"
        _SOURCE_PDF = _REF / "state_forms" / "az_140.pdf"
        ...
        fields = compute_az140_fields(state_return)
