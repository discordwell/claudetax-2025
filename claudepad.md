...
### 2026-06-19 — Landed NIIT double-count fix; fixed 1099-R income drop + EITC phase-out; warned on Social Security

Maintenance pass. Closed the NIIT double-count follow-up flagged last session, then found and fixed two more "silently wrong number" bugs via two read-only audit sub-agents, and added a loud warning for the one income type that still can't be computed. Each fix locked with tests confirmed to FAIL against pre-fix code; an adversarial code-review sub-agent fuzzed the EITC change against 200k random cases (0 mismatches vs IRS Worksheet A) and confirmed all changes correct.

**Landed first (the prior session's WIP): NIIT double-count.** tenforty computes `federal_niit` and bundles it into `federal_total_tax`, so the engine's `(federal_total_tax − federal_income_tax)` "other taxes" gap already contained tenforty's NIIT; the engine then ADDED its own Form 8960 value on top, overstating total_tax by the full NIIT for every NIIT filer (a $300k single w/ investment income: $4,700 other taxes instead of $2,800 = $1,900 NIIT + $900 Add'l Medicare). Fix strips tenforty's `federal_niit` from the gap before adding the authoritative value. Tightened the NIIT wiring tests from `>=` to exact equality so it can't silently return. (Committed separately.)

**Bug 1 — 1099-R retirement income silently dropped from AGI/tax (HIGH; every retiree with a pension/IRA distribution).** `_to_tenforty_input` summed 1099-R `box2a_taxable_amount` into the *display* `total_income` (line 9) and the renderer printed it on line 4b, but it was NEVER routed into tenforty, so it never reached AGI/taxable income/tax. A retiree with a $40k 1099-R and nothing else computed AGI $0 / tax $0, and the rendered 1040 was internally inconsistent (line 9 ≠ line 11 + deductions). Fix: route the taxable amount through tenforty's `schedule_1_income` (taxed as ordinary income → AGI), as a tenforty-input-only addition — `schedule_1_net()` stays clean so the Schedule 1 renderer still shows only true Schedule 1 items (1099-R belongs on line 4b/5b). Verified: $40k pension → AGI $40k, TI $24,250, tax $2,675; correctly cascades to NIIT MAGI (a pension lifting MAGI over $200k raised NIIT from $0 to $380) and to AGI-conforming state returns.

**Bug 2 — EITC phase-out subtracted from the phase-in credit instead of the max credit (HIGH-confidence; understates a refundable credit).** Old code: `credit = min(earned*rate, max) − phase_out_reduction(max(earned, AGI))`. IRS Pub. 596 Worksheet A is `credit = min(table(earned_income), table(AGI))` where `table` is the tent function and the phase-out applies to the *plateau max*, never to a phasing-in amount. The two diverge whenever earned income and AGI land on different parts of the tent — e.g. $5k wages + $25k IRA, 1 kid: old $637, correct $1,700. Also corrected the reverse (AGI < earned via adjustments: old overstated). Every prior test used `earned == AGI`, where the formulas coincide, so it was never caught. Replaced with the tent-function min-of-two-lookups.

**Bug 3 (not fixed — warned) — Social Security benefits omitted from taxable income.** SSA-1099 box 5 is in display `total_income` but the IRC §86 taxable-benefit worksheet (up to 85% taxable) is not implemented and line 6b is $0, so the taxable portion never reaches AGI. Implementing §86 needs a provisional-income multi-pass interaction (risky to land safely this pass), so instead added a LOUD `compute_warnings` entry (same pattern as the QBI-above-threshold warning) naming the amount, the §86 worksheet, line 6b, and the 85% ceiling — strictly better than the silent $0.

**Known follow-ups (found, deferred):** (a) K-1 ordinary business income — partnership box 1 reaches AGI via box-14 SE earnings, but S-corp K-1 box 1 (no box 14) is dropped; fixing safely needs an entity-type model field to avoid double-counting the SE path. (b) 1099-MISC box 1/2/3 never aggregated into income. (c) §86 Social Security worksheet (warned for now). (d) 1099-R early-distribution 10% additional tax (Form 5329) not computed.

**Tests:** +14 (new `test_1099r_retirement_income.py` ×6 incl. NIIT-MAGI cascade; `test_calc_eitc.py` +5 phase-in-ramp/divergence; `test_engine_warnings.py` +3 SSA). All confirmed to FAIL against pre-fix code (temporary revert + rerun). Suite **4204 → 4218 passed**, 3 skipped. No schema change. Adversarial review: all three changes CORRECT, no double-count, no Schedule 1 pollution, multi-pass consistent.

**Files:** `calc/engine.py`, `calc/patches/eitc.py`, 3 test files (+1 new).

### 2026-06-18 — OBBBA Schedule 1-A deductions moved below the line (line 13b) + age-65/blind additional standard deduction

Maintenance pass. Found via two read-only audit sub-agents + web verification against the IRS 2025 Schedule 1-A / Form 1040 draft. The single biggest "silently wrong number" class found so far: **three** deductions (senior, tips, overtime) were corrupting AGI.

**The bug (HIGH impact, very common: every 65+ filer, every tipped/overtime worker).** The engine treated the OBBBA senior deduction AND the Schedule 1-A tips/overtime deductions as Schedule 1 Part II **adjustments that reduce AGI** (`_sum_adjustments` summed all three; `compute()` did `real_agi = prelim_agi - obbba_total`). But the IRS 2025 Schedule 1-A puts all four of those deductions (tips, overtime, car-loan interest, senior) on **Form 1040 line 13b — BELOW the AGI line** — so they reduce *taxable income only, never AGI/MAGI*. Confirmed against Thomson Reuters, NATP, TurboTax, TaxSlayer, and the IRS draft (line 14 = line 12 + 13a + 13b; line 15 = line 11 AGI − line 14). A prior session's audit agent had wrongly blessed tips/overtime as "genuinely above-the-line" — the form design overrules that. Because AGI was understated, every MAGI-driven phase-out was silently corrupted: **NIIT threshold, the 7.5% medical-expense floor, education/IRA/savers phase-outs, and every state return that conforms to federal AGI.** Empirically: a MFJ-both-65 return with $255k income wrongly computed **$0 NIIT** (AGI pushed below $250k) instead of $190; a senior itemizer's medical floor used a too-low AGI and over-deducted.

**Second bug, same fix area (HIGH, every retiree on the standard deduction).** tenforty has **no age/blind input**, so the engine never applied the §63(f) age-65/blind **additional standard deduction** ($2,000/box single-HoH, $1,600/box MFJ-MFS-QSS). The senior-deduction module's docstring claimed the main engine handled it — it did not. A 65+ standard filer's taxable income was overstated by $2,000+ → overpaid ~$440 in the senior_with_tips fixture alone.

**The fix.** Unified the below-the-line deduction computation in `compute()` to mirror the existing QBI pattern: AGI stays clean (first tenforty pass, OBBBA stripped); line 12 = standard (base + age-65/blind additional, via new `_additional_standard_deduction`) or itemized (medical floor now on the TRUE AGI); line 13b = OBBBA Schedule 1-A total; line 13a = QBI; a final "Itemized" tenforty pass folds `line12 + qbi + obbba` so brackets/LTCG see the right taxable income — **AGI untouched**. New `ComputedTotals.additional_deductions_schedule_1a` (line 13b; schema regenerated). QBI's 20%-of-TI cap now uses `prelim_agi − line12 − obbba_total` = Form 8995 line 11 (subtracting line 13b — matches the IRS's 2026-01-27 corrected 8995 instructions; the old code omitted it). OBBBA removed from `_sum_adjustments`; Schedule 1 renderer no longer shows OBBBA on lines 9/25 (repurposed to the real "total other income"/"total other adjustments"); Form 1040 renderer + FFFF map + acroform map (f2_04) now carry line 13a/13b; Form 6251 comments corrected (line_1b = AGI − QBI − Sch1A is the right AMTI base — no AMT regression). Hot path (no senior/tips/QBI/medical/age/blind) stays a single tenforty pass, bit-for-bit unchanged.

**Tests.** New `test_obbba_below_the_line.py` (+14): AGI unchanged by OBBBA, NIIT + medical-floor cascades, age-65/blind additional across statuses (single/MFJ/MFS/QSS, blind, both boxes, itemizer gets none), Form 1040 13b reconciliation, idempotency, hot-path invariance. Updated the golden `senior_with_tips` fixture (AGI 69300→80000, deduction 15750→17750, TI 53550→51550, tax 6701→6261) and the integration/regression/schedule-1 tests that had codified the AGI-reducing bug. Suite **4188 → 4204 passed**, 3 skipped. Adversarial 8-point code-review sub-agent: CORRECT, no regressions (independently verified the QBI line-11 change against IRS).

**Known follow-up (separate, also found this pass): NIIT is double-counted.** tenforty computes `federal_niit` and bundles it into `federal_total_tax`; the engine then adds its own `compute_niit` result on top (`other_taxes_val = (tf_total_tax − fed_tax) + niit_val`), so total_tax is overstated by the NIIT amount for every NIIT filer. Untested end-to-end, so CI never caught it. Fix = subtract tenforty's `federal_niit` from `tf_other_taxes` before adding the engine's authoritative Form 8960 value.

**Files:** `calc/engine.py`, `models.py`, `output/form_1040.py`, `output/schedule_1.py`, `output/form_6251.py`, `output/ffff_entry_map.py`, `reference/form-1040-acroform-map.json`, `schemas/return.schema.json`, `fixtures/senior_with_tips/expected.json`, 4 test files (+1 new).
