...
# ---------------------------------------------------------------------------
# Layer 1: OR Form OR-40 field dataclass
# ---------------------------------------------------------------------------


@dataclass(frozen=True)
class OR40Fields:
    """Frozen snapshot of OR Form OR-40 line values, ready for rendering.

    Field names correspond to state_specific keys produced by
    OregonPlugin.compute().
    """
...
def compute_or40_fields(state_return: StateReturn) -> OR40Fields:
    """Map StateReturn.state_specific to OR40Fields."""
...
class OregonPlugin:
    ...
    def render_pdfs(
        ...
        _WIDGET_MAP = _REF / "or-40-acroform-map.json"
        _SOURCE_PDF = _REF / "state_forms" / "or_40.pdf"
        ...
        fields = compute_or40_fields(state_return)
