...
def _build_d400_fields(state_return: StateReturn) -> D400Fields:
    """Layer 1: map StateReturn.state_specific to D400Fields."""
    ...
    nc_withholding = ss.get("nc_withholding", _ZERO)
    ...
    return D400Fields(
        taxpayer_first_name=ss.get("_taxpayer_first_name", ""),
        taxpayer_last_name=ss.get("_taxpayer_last_name", ""),
        taxpayer_ssn=ss.get("_taxpayer_ssn", ""),
        address=ss.get("_address", ""),
        city=ss.get("_city", ""),
        state=ss.get("_state", "NC"),
        zip=ss.get("_zip", ""),
        filing_status=ss.get("_filing_status", "single"),
        line_6_federal_agi=agi,
        line_8_add_lines_6_and_7=agi,
        line_10b_subtract=agi,
        line_11_nc_standard_or_itemized=nc_deduction,
        line_12a_subtract=ti,
...
class NorthCarolinaPlugin:
    ...
    def render_pdfs(
        ...
        Layer 1: build D400Fields via _build_d400_fields factory.
        Layer 2: overlay onto the source PDF via fill_acroform_pdf.
        """
        from dataclasses import asdict

        d400 = _build_d400_fields(state_return)
        ...
        for sem_name, value in asdict(d400).items():
        ...
        if d400.filing_status in wmap.filing_status_checkboxes:
            widget_values[wmap.filing_status_checkboxes[d400.filing_status]] = True

        out_path = Path(out_dir) / "NC_D-400.pdf"
