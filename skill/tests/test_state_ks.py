...
class TestKansasPluginFormIds:
    ...
    def test_render_pdfs_produces_pdf(
        ...
        """KS Form K-40 AcroForm fill produces a non-empty PDF."""
        try:
        ...
        assert paths[0].name == "ks_k40.pdf"

    def test_render_pdfs_field_values(
        self, single_65k_return, federal_single_65k, tmp_path
    ):
        """Verify that rendered KS K-40 PDF contains correct field values."""
        try:
            from pypdf import PdfReader
        except BaseException:
            pytest.skip("pypdf unavailable")
        ...
        # state_total_tax = 2827.71 for $65k Single
        tax_field = fields.get("8 tax, use tax tables ot tax computation schedule")
        assert tax_field is not None
        assert tax_field.get("/V") == "2827.71"
